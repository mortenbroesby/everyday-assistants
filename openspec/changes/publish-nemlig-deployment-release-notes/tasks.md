...
- [x] 2.1 Add failing tests for eligible and ineligible exact ranges, missing or
  malformed notes, wrong versions, notes absent from the candidate diff, and
  bounded valid Markdown.
- [x] 2.2 Implement the minimal release-note validation by reusing the existing
  changed-file reader and package version decision; add no dependency or second
  path classifier.
- [x] 2.3 Wire the exact-base/exact-head CI version gate to require a valid note
  only for release-bearing changes; document the agent-authored note and PR
  review workflow.

## 3. Publish only verified deployments

- [x] 3.1 Add failing tests for wrong SHA/run, failed, rolled-back, incomplete,
  live-pending, and missing routine acceptance journals plus a valid journal.
- [x] 3.2 Implement deterministic journal validation and an idempotent GitHub
