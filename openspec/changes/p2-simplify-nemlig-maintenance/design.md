...
| `.github/workflows/ci.yml` previously called readiness without `check:version-bump` | Existing package-distribution version policy was not enforced in CI | Characterize the checker and reconcile its caller with #75: SHA/green-CI eligibility replaces version/codename/release-note deployment gating; retain legacy tooling until its separately scoped cleanup |
...
6. **Release/deployment identity:** Git SHA and exact trusted CI are authoritative for production eligibility. The existing version-bump checker remains an explicit legacy release-tooling command until a later cleanup, but is not a CI or deployment prerequisite. Codename metadata and runtime identity remain retained; no future removal may proceed without a separate runtime migration. No generic `HEAD~1` assumption is introduced, and unit fixtures must not push, tag or deploy.
