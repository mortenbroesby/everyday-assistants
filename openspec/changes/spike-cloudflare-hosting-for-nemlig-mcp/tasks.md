...
- [ ] 1.1 Verify current official Cloudflare Workers, Containers, Durable Objects,
...
- [ ] 1.2 Inspect the Nemlig app's engines, entry points, package and Docker setup,
  ...
  evidence.
- [ ] 1.3 Classify all state as restart-discardable, restart-required, or external;
  verify pending mutations and indeterminate results retain their existing
  fail-closed behavior.
- [ ] 1.4 Write `docs/cloudflare-hosting-assessment.md` comparing direct Workers,
...
- [ ] 2.1 Confirm whether the existing Dockerized MCP can satisfy the contract
  ...
  substantial rewrite.
- [ ] 2.2 Present the assessment, exact provider resources, expected monthly cost,
...
- [ ] 2.3 Stop before gateway or deployment implementation until the owner starts
...
- [ ] 3.1 Add the smallest current Cloudflare repository configuration for local
...
- [ ] 3.2 Implement the Worker MCP route with the exact `MCP_ENABLED === "true"`
...
- [ ] 3.3 Reuse the existing single-owner Auth0 validation before usage state and
  Container access; verify unauthenticated and unauthorized requests cannot wake
  the backend.
- [ ] 3.4 Add bounded request validation, operation classification, and forwarding
...
- [ ] 4.1 Implement the minimal global usage state with usage-period key, normal
...
- [ ] 4.2 Add configurable daily and expensive-operation quotas with conservative
...
- [ ] 4.3 Add safe next-period reset and authenticated manual reset behavior;
  verify reset does not bypass authentication and emits only a privacy-safe state
  event.
- [ ] 4.4 Add per-owner normal and expensive rate limits with starting values of
...
- [ ] 5.1 Configure measured thin-gateway CPU and subrequest limits using current
...
- [ ] 5.2 Audit and bound every Container and Nemlig request timeout and retry;
...
- [ ] 5.3 Add minimal structured events for enablement rejection, authentication
...
- [ ] 5.4 Ensure non-production lacks real Nemlig mutation credentials by default;
...
- [ ] 6.1 Add gateway tests for disabled, unauthenticated, and normal authenticated
  requests; verify disabled and unauthenticated cases never call the backend.
- [ ] 6.2 Add tests for normal rate limit, daily quota, expensive-operation quota,
  open-breaker persistence, and backend non-invocation while tripped; verify each
  failure is 429 or 503 and fail closed.
- [ ] 6.3 Add timeout and bounded-retry tests; verify the measured attempt count and
  elapsed bound cannot regenerate work after the request ends.
- [ ] 6.4 Write `docs/cloudflare-operations.md` covering deploy, immediate disable,
...
- [ ] 6.5 Run focused tests, `pnpm verify`, privacy checks, and strict OpenSpec
  validation; verify the exact commit passes without production credentials or a
  live Nemlig mutation.
- [ ] 6.6 Return the recommended architecture, implemented changes, manual
