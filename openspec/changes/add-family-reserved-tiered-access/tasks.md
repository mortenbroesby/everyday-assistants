...
- [ ] 1.1 Implement a bounded versioned principal-policy parser with unique
...
- [ ] 1.2 Recheck official Cloudflare Container, Worker, Durable Object, log, and
...
- [ ] 2.1 Return the validated Auth0 subject from the existing issuer, audience,
...
- [ ] 2.2 Resolve enabled principals at the Worker before Durable Object access
...
- [ ] 2.3 Extend the existing atomic usage record with bounded per-principal and
...
- [ ] 2.4 Keep the global kill switch, breaker, quotas, CPU/subrequest limits,
...
- [ ] 3.1 Replace the hosted shared client and proposal service with a bounded
...
- [ ] 3.2 Bind every MCP transport and proposal lifecycle to the authenticated
...
- [ ] 3.3 Scope named lists and new hosted plan snapshots by opaque principal key,
...
- [ ] 4.1 Add fixed tier and denial-reason fields to privacy-safe terminal events
  ...
  or unbounded value.
- [ ] 4.2 Document owner-only policy creation and rotation, disabled invitee
...
- [ ] 4.3 Update the feature documentation, backlog status, production acceptance,
...
- [ ] 5.1 Run focused policy, authentication, admission, concurrency, HTTP,
