...
2026-09-09 continuation: the owner answered the recommendation to use synthetic checks for routine releases, retaining real-user checks for first cutover and relevant behavior changes, with “Go ahead with remaining.” D1's release-evidence matrix is accepted. The later owner setup record and protected routine deployment establish the current D2–D4 decisions below; they do not authorize unrelated provider changes, plan upgrades, or live basket operations. The root thread coordinates at the owner's request, with Sol used only for bounded planning unknowns and Terra/Luna for implementation/review.

### Decision reconciliation — 2026-09-22

- **D1 accepted:** routine releases use exact trusted main CI plus edge and isolated machine-authenticated service acceptance. First cutover and changes affecting OAuth, principals, credentials, provider login/session, or another real-user boundary retain separately recorded live evidence. Synthetic success is never relabeled as family/provider acceptance.
- **D2 accepted for the current setup:** retain the existing API-wide token lifetime and use one bounded M2M service-token request per protected service acceptance. The successful protected run proves the configured grant can issue the required token; it does not claim an unlimited tenant allowance or authorize a plan upgrade. No API-wide lifetime change was made.
- **D3 accepted:** `nemlig-production` is the protected environment for exact `main` releases, with the recorded owner reviewer/self-review policy and no administrator bypass. Merge approval is the human release decision; a successful main push may then enter the already protected automatic workflow without a second routine approval prompt.
- **D4 accepted with an explicit residual:** use the scoped Cloudflare deployment token and account metadata recorded in `owner-setup.md`. Successful preflight, deployment, readback, and edge probe prove the required operation path works; they do not prove a Worker-only token scope, so the account-wide residual remains documented and no stronger least-privilege claim is made.
...
Routine deployment must not need a superuser login, owner access token, browser
session, or manual token copy. The proposed local-owner PKCE helper is dropped.
Proceed with the existing restricted machine-identity design in repository code,
disabled by default. Provider activation still requires verified entitlement,
least-supported scopes and secure one-time setup; those checks must not block
credential-free implementation and tests. Preserve the existing API token
lifetime unless a separately reviewed provider decision changes it.

First-cutover and affected authentication/provider changes retain real-user
acceptance, performed through the existing user client and reported separately;
that evidence must not put owner credentials into the CI runner. Missing live
proof leaves release acceptance incomplete rather than requesting an owner token
from CI. Existing explicit local owner mode may remain for diagnosis but is not
the normal deployment path. Do not reopen completed S1/S2 slices.
...
This recommendation is **selected for the proposed implementation**, disabled by default. Sol may implement identity-independent packets immediately. The owner clarification permits default-off runtime identity implementation now. Service evidence intentionally excludes live Nemlig and ChatGPT OAuth proof; D2 provider application/cost setup remains a separate activation gate. A different owner choice requires revising this design, specs, and affected packets first. Do not carry contradictory optional implementations into code.
...
A read-then-delete GitHub ref call is not a provider compare-and-delete operation. Safety depends on protocol participants never stealing/replacing a live lease, unique operation ownership, and final ref verification. A protected job may invoke the existing finalizer after its deployment command has stopped and its final artifact upload succeeds; the finalizer still requires the exact terminal journal and current Worker/Container proof, and cutovers awaiting live acceptance retain ownership. Recovery outside that same job remains manual and requires confirming the original runner and child processes are stopped. Native CI concurrency helps only CI; dashboard/other tools are not fenced. Recheck deployment IDs before every normal AND rollback mutation. If a new actor's version appears, stop and retain evidence; do not roll it back.

Cancellation/timeout can leave a Wrangler child or provider upload running after the process disappears. Register bounded best-effort termination handlers, stop/wait for child processes, but never depend on `finally`, an `always()` artifact step, or job cancellation as proof of provider state. Automatic finalization runs only after the deployment process exits, the artifact step succeeds, and the finalizer independently proves a known terminal state. If a command was sent and outcome is missing, mark unknown and retain lease. Reconcile the expected version/commit/image and recent deployments after original runner termination before retrying; timeout/504 can be late success. There is no automatic lease TTL or force-unlock.
...
## Issue #75 Phase A reconciliation

The normal workflow remains fail-closed when `main` advances between a successful CI run and the release gate; it does not silently substitute a newer revision. Protected manual recovery may select a previously green full SHA only when it is an ancestor of current `main`, has exact trusted CI, and records recovery provenance separately. This is an incident-recovery path, not a routine bypass. The codename ledger and runtime codename remain in the maintained contract because the current runtime requires them; removal is deferred until a separate identity migration. Automatic prerelease publication is no longer part of the canonical production workflow; legacy publication tooling remains outside the routine path until a later cleanup decision.

Existing provider versions may carry a legacy abbreviated Git revision from an earlier deployment. Starting-state inspection accepts only a bounded hexadecimal Git abbreviation (7–40 characters) so a safe read-only baseline can be established; every newly deployed candidate and its readback still require the exact full 40-character SHA. Routine recovery finalization also runs only after the provider deployment step itself succeeds, so an unchanged pre-mutation failure cannot trigger a misleading second finalizer failure.
