...
- [x] 0.3 Sol records D1–D4 decisions separately from technical U1–U5 evidence, updating all affected artifacts together; verify undecided provider/fixture activation remains disabled and no routine repository reapproval is requested. Decision reconciliation is recorded in `design.md` and `owner-setup.md`; the account-wide Cloudflare scope residual and synthetic/live evidence boundary remain explicit.
...
- [x] 5.4 Run a credential-free Linux workflow rehearsal using fake provider/token endpoints and the real command orchestration; verify successful trace, canceled run, same-SHA competing run, source drift and rollback failure all report the correct terminal state. Exact-head CI run `35664217966` executed `pnpm nemlig:production:ready` on `ubuntu-latest`; its 95 focused deployment/workflow tests exercise the fake command/provider/token seams and the named terminal-state cases without production credentials or provider mutation.
...
- [x] 7.4 Complete approved bounded live-user/provider and existing-app ChatGPT cutover evidence, recording actual timestamps/revision and pending/unavailable boundaries; verify no duplicate app, background-handoff assumption, prepare/apply or shopping-data write. The existing app was reconnected and its read-only favourites action passed at `2026-09-09T15:57:18Z` for deployed revision `095ef33072b1290a1b316c4fb45c334669d08736`; no duplicate app or write action was used.
- [x] 7.5 Record measured CI duration, token calls, request counts and Container state against the initial cost budget, and verify recovery evidence; if a live rollback rehearsal is separately approved, execute and verify exact Worker/image restoration or honest disabled/unknown state. Run `35664402066` used one service-token issuance, one bounded service-fixture acceptance, one read-only edge probe with five correlation IDs, one candidate image, and one Container application; the release job ran from `22:47:28Z` to `22:50:28Z`, with the deploy command from `22:47:53Z` to `22:50:09Z`. The report recorded application version 74, image `sha256:c6280f16f769c88dfcadbf731d2e514cdbb0aedc290e0b3f66b0d1b8f45e0d3b`, and no rollback. Fake recovery tests cover cancellation, late success, provider drift, journal failure, and retained ownership. No live rollback rehearsal was performed because it is a separate provider mutation and no exact rollback window was authorized.
- [x] 7.6 Sol updates operations/readiness docs and predecessor evidence only where actual acceptance satisfies their tasks; verify source/SHA/profile separation and no historical result is rewritten as current. `docs/cloudflare-operations.md` now records the protected routine release, automatic routine finalization and retained cutover recovery sequence while keeping deployed and repository SHAs distinct.
...
- [x] 8.1 Reconcile older hosting deltas and their remaining live tasks, sync/archive completed predecessors in order, then this capability; verify strict validation and no conflicting, duplicated or silently waived requirement. The completed hosting/readiness/deployment predecessors are archived; active P0 onboarding, codename identity, and P2 maintenance changes retain their own unresolved live or follow-up tasks and are not silently closed. This change records the current D1–D4 decisions, synthetic/live boundary, and exact deployed-vs-repository SHA distinction.
- [x] 8.2 Run required final repository gates, commit/push archival changes, verify exact remote main/CI and state last deployed application SHA separately from archival SHA; deliver durable evidence links, remaining owner operations if any, and jCodeMunch usage without invented savings. Closeout documentation merged at main SHA `a27c001327e3518a204d116ae01e1a242f46152c`; exact-main CI run `35666106170` passed. Last deployed application SHA remains `d5e62e6d5259e50ff668d26652a977009add565d` from protected run `35664402066`; the documentation merge triggered no provider mutation. jCodeMunch was used during initial repository investigation; no token-savings claim is made.
...
paths are covered by the workflow and production-deploy tests (95/95 focused
