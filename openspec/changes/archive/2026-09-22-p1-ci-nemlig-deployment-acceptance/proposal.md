...
Production eligibility is Git-SHA and exact-green-CI based. Routine delivery requires the current `main` head; a separately selected protected recovery mode may deploy a previously green main ancestor with explicit recovery provenance. Human-facing codename metadata and its runtime dependency remain retained; codename-ledger/runtime removal is deferred. The automatic publication job is removed from the canonical workflow, while remaining release/publication scripts are legacy tooling pending a later cleanup or guard.
