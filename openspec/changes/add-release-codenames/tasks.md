...
- [x] 2.4 Require new codenamed release notes to lead with a bounded `In plain language` section, update the Callsign note, and verify malformed or missing plain-language summaries fail before publication.
...
- [x] 3.3 Add one shared release glossary for aliases, acronyms, and difficult terms, and link it from the release authoring and operations guidance.

## 4. Release and delivery

- [x] 4.1 Re-run the release planner from the refreshed main baseline with reviewed codename `Callsign`, apply its final plain version, replace the stale candidate note with the matching bounded note, and verify the exact candidate passes version and note gates without tagging, publishing, or deploying.
...
- [x] 4.3 Review the final diff for unchanged safety/cost controls, update and push the scoped pull request, and verify the remote head and exact-head CI; do not merge or deploy without the separately applicable protected workflow decision.
