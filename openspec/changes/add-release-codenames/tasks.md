...
- [ ] 4.1 Re-run the release planner from the refreshed main baseline with reviewed codename `Callsign`, apply its final plain version, replace the stale candidate note with the matching bounded note, and verify the exact candidate passes version and note gates without tagging, publishing, or deploying.
