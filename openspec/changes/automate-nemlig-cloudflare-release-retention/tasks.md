...
- [x] 3.3 Add accepted-image order ledger and pure planner: explicit one-time reset of pre-reset images in the exact approved repository after accepted baseline; then fifty distinct accepted digests by default, with active/recovery/uncertain references preserved.
...
- [ ] 4.3 After the unresolved production journal is safely reconciled, verify a live post-acceptance cleanup reaches the fifty-image target or reports protected/uncertain holds. No production deletion has been run by this branch.
...
- [x] 5.2 Add/adjust failure-injection and workflow-contract tests; document exact deploy, acceptance, retention, uncertainty and explicit resume behavior.
