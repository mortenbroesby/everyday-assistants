...
independent implementations, not a target file size or exactly six MCP tools.
...
ordinary planning. Correct and pin that wording in a separate small documentation/
metadata slice; do not change discovery behavior to match the stale description.

## Product model: six journeys, explicit effect boundaries
...
tool. A giant `shopping_list(action=...)` would mix storage-only reads, persistence
writes and live provider work. Group operations only when their contracts and host
behavior remain understandable. Start by removing duplicate user decisions, then
...
| 2 | Prefer named lists and reduce duplicate choices — medium | 0; reuse 1 where useful | Specify save/open/refresh semantics and old-plan compatibility. Demonstrate fewer decisions in fresh-list and saved-list fixtures without extra Nemlig calls. Test old IDs, ownership, revisions and archive/restore. No deletion or live migration yet. |
...
additions; saved-list open/refresh; stale list revision; changed price; declined
...
storage-only list opening makes zero Nemlig requests; discovery budgets do not
